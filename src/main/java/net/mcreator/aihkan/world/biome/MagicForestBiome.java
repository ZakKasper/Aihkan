...
import net.mcreator.aihkan.block.MagicWoodBlock;
import net.mcreator.aihkan.block.MagicLeavesBlock;
...
public class MagicForestBiome extends AihkanModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(MagicWoodBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(MagicLeavesBlock.block.getDefaultState()))).baseHeight(9)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MagicGrassBlockBlock.block.getDefaultState().getBlock() || ground == Blocks.STONE.getDefaultState().getBlock())
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MagicWoodBlock.block.getDefaultState().getBlock()
					|| blockType == MagicLeavesBlock.block.getDefaultState().getBlock()
					|| blockType == MagicGrassBlockBlock.block.getDefaultState().getBlock() || blockType == Blocks.STONE.getDefaultState().getBlock();
		}
