	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MagicGrassBlockBlock.block.getDefaultState().getBlock()
							|| ground == MagicDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == MagicGrassBlockBlock.block.getDefaultState().getBlock()
									|| ground2 == MagicDirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), MagicDirtBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MagicWoodBlock.block.getDefaultState().getBlock()
					...
					|| blockType == MagicDirtBlock.block.getDefaultState().getBlock();
		}
