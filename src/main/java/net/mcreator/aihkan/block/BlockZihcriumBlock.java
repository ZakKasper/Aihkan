...
import net.mcreator.aihkan.item.DropZihcriumItem;
...
public class BlockZihcriumBlock extends AihkanModElements.ModElement {
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("block_zihcrium", "block_zihcrium", blockAt -> {
			...
			}), block.getDefaultState(), 6)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(6, 12, 12, 34))));
		}
	}
}
